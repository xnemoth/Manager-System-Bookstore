...
namespace QuanLyCuaHangSach
{
    class Functions
    {
        ...
        public static void Connect()
        {
            ...
            Con.ConnectionString = @"Data Source=.\SQLEXPRESS;AttachDbFilename=C:\Users\CNC\Documents\Visual Studio 2010\Projects\QuanLyCuaHangSach\QuanLyCuaHangSach\bin\Debug\QLCHS.mdf;Integrated Security=True;Connect Timeout=30;User Instance=True";
            Con.Open();                  //Mở kết nối
            //Kiểm tra kết nối
        ...
        public static string checkUser(string username, string pass)
        {
            ...
            try
            {
                String sql =  ("SELECT MaNhanVien, TenNhanVien, Password,Quyen FROM T02_NhanVien WHERE MaNhanVien =N'" + username + "' and Password='" + pass + "'");
        ...
        public static string CreateKeyHD(string tiento)
        {
            string key = tiento;
            string[] partsDay;
        ...
        public static string createKeyList(string Loai)
        {
            string key;
            if (Loai.IndexOf(' ') > -1)
            {
                string[] list = Loai.Split(' ');
                list[0] = list[0].Substring(0, 1);
                list[1] = list[1].Substring(0, 1);
                key = list[0] + list[1];
                return key;
            }
            else
            {
                try
                {
                    key = Loai.Substring(0, 2);
                    return key;
                }
                catch
                {
                    key = Loai.Substring(0, 1);
                    return key;
                }
            }
        }
